...
func_visual_behavior <- function(df_name, variable_name, question_text) {
  df <- tabyl(df_name, behavior_personal) %>% 
    rename(choice = behavior_personal) %>% 
    mutate(bar_color = case_when(choice == df_name %>% filter(timestamp == max(timestamp)) %>% select(behavior_personal) %>% as.character() ~ "#e88f00",
                                 TRUE ~ "#124d47")) %>% 
    mutate(your_selection = case_when(choice == df_name %>% filter(timestamp == max(timestamp)) %>% select(behavior_personal) %>% as.character() ~ paste0(percent(percent, accuracy = 1), " (Your pick)"),
                                      TRUE ~ percent(percent, accuracy = 1)))
